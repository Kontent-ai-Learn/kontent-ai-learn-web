...
(() => {
  ...
  const zoomItem = (elemSelector, basicLightboxInstance, content, figcaption, callback, link, title) => {
    ...
    basicLightboxInstance = window.basicLightbox.create(`<div class="basicLightbox__content-container">${closeMarkup}${content}${figcaption}</div>`, {
      onShow: () => {
        if (callback && link) callback(link, title);
      },
      onClose: () => {
        if (callback) callback(null, title);
      }
    });
    ...
    setTimeout(() => {
      const initLightbox = () => {
        document.querySelectorAll('[data-lightbox-image]').forEach((item) => {
          // Find caption in DOM generated by Kontent.ai
  ...
  const initLightboxOnLandingPage = () => {
    ...
    const handleUpdate = (path, title) => {
      const url = path || pathRoot || '';
      window.history.pushState('', '', url);
      if (title) {
        if (path) {
          title.titleElem.innerHTML = `${title.titleDetail} |${title.suffix}`;
        } else {
          title.titleElem.innerHTML = `${title.titleOverview} |${title.suffix}`;
        }
      }
    ...
    const getTitle = (titleDetail) => {
      const titleElem = document.querySelector('title');
      const titleLpElem = document.querySelector('[data-lp-title]');
      let titleSuffix = '';
      let titleLp = ''
      if (titleElem) {
        const titleElemSegments = titleElem.innerHTML.split('|');
        titleLp = titleElemSegments[0];
    ...
    const handleLightBox = (e) => {
      ...
      const titleTextElem = title ? title.querySelector('.card__title-text') : null;
      ...
      let instance;
      const titleText = titleTextElem ? titleTextElem.innerHTML.trim() : '';
      instance = zoomItem('.card', instance, wrap.innerHTML, '', callback, linkUrl, getTitle(titleText));
      return instance;
    };
